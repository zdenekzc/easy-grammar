...
from easy_lexer     import Lexer, LexerException, fileNameToIndex, indexToFileName
from easy_grammar   import Grammar, Rule, Expression, Alternative, Ebnf, Nonterminal, Terminal
from easy_symbols   import initSymbols
from easy_toparser  import ToParser
from easy_toproduct import ToProduct
from easy_cparser   import CParser
    ...
    def highlightBlock (self, text) :
        try :
           ...
           while not lexer.isEndOfSource () :
              ...
              length = lexer.charByteOfs - lexer.tokenByteOfs + 1
...
class MainWin (QMainWindow) :
    ...
    def onItemActivated (self, item, column) :
        if hasattr (item, "src_file") :
           fileName = indexToFileName (item.src_file)
           edit = None
           cnt = self.tabWidget.count ()
           inx = 0
           while inx < cnt :
               widget = self.tabWidget.widget (inx)
               if getattr (widget, "fileName", "") == fileName :
                  edit = widget
               inx = inx + 1
           if edit != None :
              self.tabWidget.setCurrentWidget (edit)
    ...
    def readFile (self, edit, fileName) :
        ...
        if edit == None :
           ...
           if self.extra != None :
              self.tabWidget.removeTab (self.tabWidget.indexOf (self.extra))
              self.extra = None
    ...
    def createParser (self) :
        ...
        if edit != None :
           if 1 : # try :
              ...
              parserFileName = self.outputFileName ("parser.py")
              productFileName = self.outputFileName ("product.py")
              cparserFileName = self.outputFileName ("parser.cpp")

              to_parser = ToParser ()
              to_parser.open (parserFileName)
              to_parser.parserFromGrammar (grammar)
              to_parser.close ()
              print ("parser O.K.")
              e1 = self.readFile (None, parserFileName)

              to_product = ToProduct ()
              to_product.open (productFileName)
              to_product.productFromGrammar (grammar, "parser")
              to_product.close ()
              print ("product O.K.")
              e2 = self.readFile (None, productFileName)

              c_parser = CParser ()
              c_parser.open (cparserFileName)
              c_parser.parserFromGrammar (grammar)
              c_parser.close ()
              print ("c-parser O.K.")
              self.readFile (None, cparserFileName)

              self.grammarTree (grammar, edit.fileName)
              self.pythonTree (e1, "parser")
              self.pythonTree (e2, "product")

           if 0 : # except LexerException as e :
    ...
    def runParser (self) :
        ...
        if edit != None :
           try :
              ...
              parser_module = self.loadModule (parserFileName)
              product_module = self.loadModule (productFileName)
    ...
    def grammarTree (self, grammar, fileName) :
        file_inx = fileNameToIndex (fileName)
        top = TreeItem (self.tree, "grammar")
        top.addIcon ("folder")
        top.src_file = file_inx
        self.addSymbols (top, grammar)
        for rule in grammar.rules :
           self.addBranch (top, rule, grammar, file_inx)
    ...
    def addBranch (self, above, node, grammar, file_inx) :
        ...
        if hasattr (node, "src_file") :
           item.src_file = node.src_file
        ...
        if isinstance (node, Rule) :
           ...
           self.addBranch (item, node.expr, grammar, file_inx)
        elif isinstance (node, Expression) :
           for t in node.alternatives :
              self.addBranch (item, t, grammar, file_inx)
        elif isinstance (node, Alternative) :
           for t in node.items :
              self.addBranch (item, t, grammar, file_inx)
        elif isinstance (node, Ebnf) :
    ...
    def syntaxTree (self, data) :
        ...
        top.addIcon ("folder")
        self.showSyntaxBranch (top, "", data)

    def showSyntaxBranch (self, above, name, data) :
        if name != "" :
           name = name + " : "
        ...
        if data == None :
           ...
           if hasattr (data, "src_file") :
              branch.src_file = data.src_file
           if hasattr (data, "src_line") :
              branch.src_line = data.src_line
           # branch.obj = data # show data object in property window
           for member in inspect.getmembers (data) :
               (ident, obj) = member
               if ident.startswith ("__") :
                  pass
               elif not inspect.iscode (obj) :
                  if ident != "items" :
                     self.showSyntaxBranch (branch, ident, obj)

           if hasattr (data, "items") :
              for item in data.items :
                  self.showSyntaxBranch (branch, "", item)

    # ----------------------------------------------------------------------

    def pythonTree (self, edit, title) :
        file_inx = fileNameToIndex (edit.fileName)
        source = edit.toPlainText ()
...
win = MainWin ()
win.setFont (QFont ("Sans Serif", 12))
# win.setFont (QFont ("Sans Serif", 24))
win.show ()

# win.readFile (None, "./input/cecko2.g")
# win.createParser ()
# e2 = win.readFile (None, "./input/c2.cc")
# win.tabWidget.setCurrentWidget (e2)
# win.runParser ()

win.readFile (None, "./input/cecko3.g")
# win.createParser ()
e2 = win.readFile (None, "./input/c3.cc")
win.tabWidget.setCurrentWidget (e2)
# win.runParser ()
